...
class OpenAISTTAPI:
    ...
    async def post_speech_to_text(self, file_path) -> dict | int:
        ...
        logger.debug('[green]Requesting STT')

        data = aiohttp.FormData()
        data.add_field('model', 'whisper-1')
        data.add_field('file', open(file_path, 'rb'))
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(self.sst_config['stt_timeout'])) as session:
                async with session.post(url, headers=headers, data=data) as response:
                    logger.debug('[green]Got STT Response')
