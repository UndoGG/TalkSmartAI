...
class OpenAITTSAPI:
    ...
    async def post_text_to_speech(self, text) -> str | int:
        ...
        logger.debug('[green]Requesting TTS')

        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(self.tts_config['tts_timeout'] + len(text))) as session:
                async with session.post(url, headers=headers, json=data) as response:
                    logger.debug('[green]Got TTS Response')
